    ...
    try {
      ...
      if (data.html) {
        ...
        return {
          ...
          username: nameMatch ? nameMatch[1] : this.twitterHandle,
        };
      }
    } catch {
    }
    ...
    for (const entry of entries) {
      ...
      if (idMatch && textMatch) {
        tweets.push({
          ...
          authorName: nameMatch ? this.decodeHtml(nameMatch[1]) : this.twitterHandle,
          authorUsername: usernameMatch ? usernameMatch[1] : this.twitterHandle,
          authorAvatar: avatarMatch ? avatarMatch[1] : undefined,
          createdAt: timeMatch ? timeMatch[1] : new Date().toISOString(),
          url: `https://twitter.com/${this.twitterHandle}/status/${idMatch[1]}`,
        });
      }
    }
